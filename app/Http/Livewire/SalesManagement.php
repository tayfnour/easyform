...
use App\MyClass\saleManageExctend;
...
class SalesManagement extends Component
{
    ...
    // array of forms
    //,'customers' , 'operations', 'entryheads', 'entries', 'trialbalances', 'Create New Product'

    public function mount($formAll = ['invoice_headers' , 'customers' , 'receipts' ,'invoices', 'entryheads' , 'entries' , 'Create New Product' ,  'trialbalances' ])
    {
    ...
    public function formsLoop()
    {
        ...
        foreach ($this->forms as $form) {
            ...
            if ($action == 'getParentRefrence') {
                ...
                //  show massege if errors in livewire is consept same this :
                ...
                if(!isset($this->refs[$opts[$pk]["param1"]][$opts[$pk]["param2"]]))
                    ...
                    $rows = DB::table($opts[$pk]["param3"])->where($opts[$pk]["param4"], $refVal)->get();

                        if ($rows->Count() > 0) {

                           
                            $this->fillMultipleRow($opts, $rows, "group");

                        } else {

                        // if( $fn == "invoices")  dd("rows is 0 :" . $fn);

                        //    المشكلة بعتقد انه لمل عم أعمل  السجل الافتراضي ماعم  حط المرجع الاب

                        $arr = ['entries' ,  'customers']; // not generate default row if empty
                        ...
                        // entries  Always related  to parent its table child not  independent
                            ...
                            $this->msgs[] ="لايمكن عمل سجل تلقائي لان الحقل الرئيسي يختار من الفرعي أو يجب تحديد جدول اللوكاب اولا" .$fn;
        ...
        foreach ($dataRows as $r => $rw) {
         ...
         // $rw row of database
            ...
            foreach ($opts["cols"] as $col) {
                ...
                if ($opts[$col]["param1"] == "setParentRefOnView") {
                  
                     $this->refs[$fn][$col] = $rw->$col;
                 
                } 

                if( $opts["fn"] == 'invoices' && $col == 'product_id'){
    ...
    public function updatedColArr($val, $RowColName)
    {
      ...
      //  dd($val, $RowColName);
       ...
       // dd($row_id, $col);
        ...
        $this->saveRowOnUpdate($row_id, $col , $val);
    ...
    public function saveRowOnUpdate($row_id, $col )
    {
      ...
      //  dd($this->colArr[$row_id]);
        ...
        $upArr = $this->colArr[$row_id];

        unset($upArr[$pk]);

      //  dd($row_id ,$col, $table ,$pk ,  $id , $upArr);

        try {

            DB::table($table)->where($pk, $id)->update($upArr);
    ...
    public function createNewFormRow($row_id, $frName = null , $overCol=null , $overVal=null )
    {
        ...
        // المرجع اما ان يكون  أبا  مثل هنا  او تابعا
            ...
            if (isset($opts[$col]["action"]) && $opts[$col]["action"] == 'getParentRef') {
                ...
                if(!isset($ref)) dd("you must get Main Refrence  at 465");
    ...
    //    if($table == "invoices") dd($this->colArr[$row]);
    ...
    public function setSelectValue($row_id, $colName, $k, $v)
    {
        ...
        if ($colName == "product_id") {
            ...
            $this->colArr[$row_id]["price"] = $proDetails->selprice;
    ...
    {
      //  dd($this->refs);
        return view('livewire.sales-management' ,  ['bs_arrows' => $this->arrows]);
    }
}

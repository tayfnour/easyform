...
use App\MyClass\salesPointsExt;
use Illuminate\Support\Facades\Log;


class SalesPoints extends Component
{   
    ...
    protected $listeners = ["savePosInvoice"=> "savePosInvoice" ,"setDate" => "setDate" , "refreshcomp" => "refreshcomp"];
    ...
    public function init()
    {
        ...
        $res = DB::table("myprojects")->where("isActive" ,"A")->where("proPage" , end($cc))->orderBy("ordering", "asc")->get();

        foreach ($res as $k => $v) {
            $this->forms[] = $v->proForm;
            $this->fOpts[$v->proForm] = ["proForm"=>$v->proForm,"isOpen"=>$v->isOpen , "isFloat"=>$v->isFloat, "description"=>$v->description ];
        }

      //  Log::channel('mht')->info("After Get of form Options : " . (microtime(true) - $this->startTime));


      //dd($this->fOpts);
        $this->excuteInialQueries();
        ...
        foreach ($this->forms as $form) {
            ...
            $this->formOpts[$form]["fn"] = $fn= reset($this->formOpts[$form])["formName"];
           ...
           // dd( $this->formOpts[$form]["bf"]);
            ...
            if (!empty($attrs)) {
                ...
                foreach ($attrs as $key => $value) {
                    if($key =="dynamicQuery"){
                        $this->dynamicQuery[$fn]=explode("|",$value);
                    }
    ...
    public function formsLoop()
    {
        ...
        Log::channel('mht')->info("start of Loop : " . (microtime(true) - $this->startTime));
        $this->startTime = microtime(true);
        Log::channel('mht')->info("start of Loop : " .$this->startTime);
        ...
        foreach ($this->forms as $form) {
            ...
            if ($action == 'getParentRefrence') {               
                ...
                if(!isset($this->refs[$opts[$pk]["param1"]][$opts[$pk]["param2"]]))
                    ...
                    $this->arrows[$fn] = DB::table($fn)->where($opts[$pk]["param3"],$refVal)->paginate($opts[$pk]["param4"], ['*'], $fn);;

                        if ($this->arrows[$page]->total() > 0) {

                            $this->fillMultipleRow($opts,  $this->arrows[$fn]);
                            ...
                            // entries  Always related  to parent its table child not  independent
                                ...
                                $this->msgs[] = "لايوجد مصدر للسجل الاب :" . $opts[$pk]["param1"]."/".$opts[$pk]["param2"];
                            }
              ...
              if (isset($opts["sourceQuery"]) && $opts["sourceQuery"] !==""){    
                   ...
                   if(count($argArr)==5){
                    ...
                    $this->arrows[$page]= DB::table($table)
                                            ->where($argArr[1],$argArr[2])
                                            ->orderBy($argArr[3], $argArr[4])
                                            ->paginate($argArr[0], ['*'], $page);
               ...
               if(isset($opts["dynamicQuery"])){

                 $dq= $this->dynamicQuery[$fn];

                    $this->arrows[$page] = DB::table($table)->where($dq[1] ,$dq [2])->paginate($dq[0], ['*'], $page);
               ...
               if(isset($opts["sourceQuery"]) && $opts["sourceQuery"]==""){
                    ...
                    $this->rows = $this->rows + 1;
                        ...
                        $this->formOpts[$fn][$pk]["form_translate"] .= "(Not Yet data linking)";
             ...
             Log::channel('mht')->info("End of Loop : " . (microtime(true) - $this->startTime));
             $this->startTime = microtime(true);
             Log::channel('mht')->info("End of Loop : " .$this->startTime);
        // dd($this->refs);
 ...
 //   dd($this->colArr[$row_id]);
    ...
    public function render()
    {
        ...
        return view('livewire.sales-points'  , 
        ['bs_arrows' => $this->arrows,

...
class NewTemplate extends Component
{
    ...
    public function init()
    {
        ...
        foreach ($res as $k => $v) {
            ...
            $this->fOpts[$v->proForm] = ["isOpen"=>$v->isOpen , "isFloat"=>$v->isFloat   , "description"=>$v->description ];
        }
    ...
    public function formsLoop()
    {
        ...
        foreach ($this->forms as $form) {
            ...
            if ($action == 'getParentRefrence') {               
                ...
                if (isset($opts["sourceQuery"])){                 
               
                    $this->arrows[$page]=  DB::table($table)->orderBy($pk, "desc")->paginate($opts["sourceQuery"], ['*'], $page);
                     if ($this->arrows[$page]->total() > 0) { 
                       $this->fillMultipleRow($opts, $this->arrows[$page]);
                     }
               }
 ...
 //   dd($this->colArr[$row_id]);
    ...
    public function render()
    {
        ...
        return view('livewire.new-template'  , ['bs_arrows' => $this->arrows]);
    }
}
